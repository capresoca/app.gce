<template>
  ...
  >
    ...
    <v-toolbar :dense="!contratoOriginal.id">
      <v-tooltip top class="mr-2" v-if="contratoOriginal.id && !floatMenu">
        <v-btn
          slot="activator"
          icon
      ...
      <v-list-tile v-else>
        <v-list-tile-content>
          <v-list-tile-title v-if="contratoOriginal.entidad" v-html="contratoOriginal.entidad.tercero.nombre_completo"></v-list-tile-title>
          <v-list-tile-sub-title v-html="'Código de habilitación: ' + contratoOriginal.entidad.cod_habilitacion"></v-list-tile-sub-title>
        </v-list-tile-content>
      </v-list-tile>
      <template>
        <v-spacer/>
    ...
    <v-layout row wrap>
      <v-slide-x-reverse-transition>
        <v-flex sm12 md3 v-if="contratoOriginal.id && !floatMenu">
      ...
      <v-flex :md9="contratoOriginal.id && !floatMenu">
        <v-window
          v-model="window"
          class="elevation-1"
          vertical
        >
          ...
          >
            <v-card flat class="content-list-p0 pt-0">
              <v-list v-if="contratoOriginal.id" two-line>
                <template>
                  <v-list-tile>
                    <v-list-tile-avatar :color="item.props.avatarColor">
                      <v-icon dark>{{ item.props.icon }}</v-icon>
                    ...
                    <v-list-tile-action style="width: 120px !important;" v-if="(n > 1 && n < 5) && !contrato.actaInicio">
              ...
              <v-card-text>
                <component :ref="'dataComponent' + n" :is="item.component" v-model="contrato" @clean="refresh"/>
              </v-card-text>
              <template v-if="n === 0">
                <v-divider/>
                <v-card-actions v-if="!contrato.actaInicio">
  ...
  export default {
    ...
    data: () => ({
      ...
      makeContrato: {
        ...
        estado: 'Registrado',
        gs_usuario_id: null,
        actaInicio: undefined
      },
      window: 0,
      items: [
        { props: {icon: 'work', avatarColor: 'primary', title: 'Información básica', subTitle: 'Datos básicos de la contrato.'}, component: () => import('@/components/contratos/registroContrato/RegistroDatosBasicos') },
        { props: {icon: 'verified_user', avatarColor: 'success', title: 'Servicios contratados', subTitle: 'Registro de servicios contratados'}, component: () => import('@/components/contratos/registroContrato/RegistroServicios') },
        { props: {icon: 'healing', avatarColor: 'danger', title: 'Procedimientos', subTitle: 'Registro de CUPS'}, component: () => import('@/components/contratos/registroContrato/RegistroProcedimientos') },
        { props: {icon: 'colorize', avatarColor: 'purple', title: 'Medicamentos', subTitle: 'Registro de CUMS'}, component: () => import('@/components/contratos/registroContrato/RegistroMedicamentos') },
        { props: {icon: 'category', avatarColor: 'orange', title: 'Otros bienes y servicios', subTitle: 'Otros bienes y servicios'}, component: () => import('@/components/contratos/registroContrato/RegistroOtros') },
        { props: {icon: 'how_to_vote', avatarColor: 'teal', title: 'Novedades', subTitle: 'Registro de novedades'}, component: () => import('@/components/contratos/registroContrato/RegistroNovedades') },
        { props: {icon: 'settings_input_composite', avatarColor: 'accent', title: 'Interfaz contable', subTitle: 'Configuración interfaz contable'}, component: () => import('@/components/contratos/registroContrato/RegistroInterfazContable') }
      ],
      archivoMasivo: null,
      makeArchivoMasivo: {
        loading: false,
    ...
    methods: {
      async submitArchivo () {
        if (await this.$refs.archivoMasivo.validate()) {
          ...
          this.axios.post(`contratos/${this.contrato.id}/${this.archivoMasivo.ruta}`, data)
            .then(response => {
      ...
      reloadSection (n) {
        switch (n) {
          ...
          default: {
            this.getRegistro(this.contrato.id)
          }
        }
      },
      async submit () {
        if (await this.$refs['dataComponent0'][0].validate()) {
          this.loading = true
          const typeRequest = this.contrato.id ? 'editar' : 'crear'
          let request = typeRequest === 'editar' ? this.axios.put(`contratos/${this.contrato.id}`, this.contrato) : this.axios.post('contratos', this.contrato)
          request
            .then(response => {
              console.log('el respose del contrato', response)
              this.contrato = response.data.data
              this.contratoOriginal = response.data.data
              this.$store.commit(ENTIDAD_RELOAD_ITEM, {item: JSON.parse(JSON.stringify(response.data.data)), estado: typeRequest, key: this.parametros.key})
              this.$store.commit(SNACK_SHOW, {msg: 'Los datos del contrato se han guardado satisfactoriamente. ', color: 'success'})
              this.loading = false
            }).catch(e => {
      ...
      getRegistro (id) {
        ...
        this.axios.get('contratos/' + id)
          .then((response) => {
            if (response.data !== '') {
              console.log('el contrato', response)
              response.data.data.fecha_contrato = this.moment(response.data.data.fecha_contrato).format('YYYY-MM-DD')
              response.data.data.actaInicio = response.data.data.novedades.find(x => x.tipo === 'Acta Inicio')
