...
<script>
  ...
  export default {
    ...
    props: ['value'],
    components: {
      CkeditorTextarea: () => import('@/components/general/CkeditorTextarea'),
      Loading,
    ...
    methods: {
      ...
      registroItem () {
        this.loading = true
        this.item.ce_proconminuta_id = this.value.id
      ...
      refreshNovedad () {
        ...
        if (!this.value.fecha_acta_inicio) this.item.tipo = 'Acta Inicio'
      }
    }
  }
</script>
