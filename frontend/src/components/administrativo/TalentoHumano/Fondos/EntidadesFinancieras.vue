<template>
  <div>
            ...
            <v-layout row wrap>
              ...
              <v-flex xs12>
                <v-autocomplete
                  ...
                  v-model="entidades.clase_fondo"
                  :items="complementos.claseentidad"
              ...
              <v-flex xs12 md6 lg6>
                <v-text-field v-model="entidades.codigo"
                              label="Código" key="codigo"
                              name="codigo" v-validate="'required'" required
                              :error-messages="errors.collect('codigo')"></v-text-field>                 
              </v-flex>

              <v-flex xs12 md6 lg6>
                <v-text-field v-model="entidades.descripcion"
              ...
              <v-flex xs12 md6 lg6>
                <v-text-field v-model="entidades.nit"
                              label="NIT " key="nit"
              ...
              <v-flex xs12 md6 lg6>
                <v-text-field v-model="entidades.digito_verificacion"
                              label="Dígito verificación " key="digito_verificacion"
                              name="digito verificacion" v-validate="'|max:1|'" 
                              :error-messages="errors.collect('digito verificacion')"></v-text-field>                 
              </v-flex>

              <v-flex xs12 md6 lg6>
                <v-text-field v-model="entidades.direccion"
                              label="Dirección " key="direccion"
                              name="direccion" v-validate="'|max:200|'" 
                              :error-messages="errors.collect('direccion')"></v-text-field>                 
              </v-flex>

              <v-flex xs12 md6 lg6>
                <v-text-field v-model="entidades.telefono"
                              label="Teléfono " key="telefono"
                              name="telefono" v-validate="'|max:50|'" 
    ...
    <v-card>
      <toolbar-list :info="infoComponent" title="Entidad Financiera" subtitle="Registro y gestión" btnplus btnplustitle="Crear registro" btnplustruncate @click="dialog = true"/>
      <v-container fluid>
        <v-layout row wrap>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      modalTitulo () {
        return !this.entidades.fondo ? 'Nueva entidad' : 'Editar entidad: ' + this.entidades.fondo
      },
      complementos () {
        return JSON.parse(JSON.stringify(store.getters.complementosEntidadesFinancieras))
      },
      pages () {
    ...
    methods: {
      reloadPage () {
        ...
        this.axios.get('fondo?per_page=' + this.pagination.per_page + '&page=' + this.pagination.current_page + '&search=%25' + this.search + '%25' + '&segm=' + this.segmento)
          .then((response) => {
            ...
            this.entidadesF = response.data.data
      ...
      eliminar (id) {
        this.localLoading = true
        this.axios.delete('fondo/eliminar/' + id)
          .then(res => {
      ...
      editar (item) {
        this.dialog = true
        this.entidades = Object.assign({}, item)
      },
      close () {
        this.dialog = false
        this.formReset()
      ...
      async save () {
        if (await this.validador('formPago')) {
          this.dialog = false
          this.localLoading = true
          var send = !this.entidades.fondo ? this.axios.post('fondo/crear', this.entidades) : this.axios.put('fondo/actualizar/' + this.entidades.fondo, this.entidades)
          send.then(response => {
            this.localLoading = false
            if (this.entidades.fondo) {
