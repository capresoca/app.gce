<template>
  <div>
        ...
        <v-card>
          ...
          <v-container fluid grid-list-md>
            <v-layout row wrap>
              ...
              <v-flex xs12>
                <v-text-field v-model="claseF.descripcion"
    ...
    <v-card>
      <toolbar-list :info="infoComponent" title="Clase fondo" subtitle="Registro y gestión" btnplus btnplustitle="Crear registro" btnplustruncate @click="dialog = true"/>
      <v-container fluid>
        <v-layout row wrap>
        ...
        <v-data-table
          ...
          :total-items="claseFondos.length"
          :search="search"
          hide-actions
          class="elevation-1">
          <template slot="items" slot-scope="props">
            <td>{{ props.item.clase_fondo}}</td>
            <td>{{ props.item.descripcion}}</td>
            <td v-if="props.item.tipo_fondo == 1">{{ 'Fondo' }}</td>
            <td v-if="props.item.tipo_fondo == 2">{{ 'Entidad financiera' }}</td>
            <td>
              <v-speed-dial
              ...
              >
                ...
                <v-tooltip top>
                  <v-btn
                    ...
                    color="white"
                    slot="activator"
                    @click="eliminar(props.item.clase_fondo)"
                  >
...
<script>
  ...
  export default {
    ...
    computed: {
      infoComponent () {
        return this.$store.getters.getInfoComponent('claseFondo')
      },
      modalTitulo () {
        return !this.claseF.clase_fondo ? 'Nueva clase de fondo' : 'Editar clase de fondo: ' + this.claseF.clase_fondo
      },
      pages () {
    ...
    methods: {
      reloadPage () {
        ...
        this.axios.get('clasefondo?per_page=' + this.pagination.per_page + '&page=' + this.pagination.current_page + '&search=%25' + this.search + '%25')
          .then((response) => {
            ...
            this.claseFondos = response.data.data
      ...
      confirmaAnulacion (motivo) {
        this.localLoading = true
        this.axios.delete('clasefondo/eliminar/' + this.dialogA.registroID)
          .then(res => {
            this.$store.commit(SNACK_SHOW, {msg: 'La clase de fondo se eliminó correctamente', color: 'success'})
            this.reloadPage()
      ...
      editar (item) {
        this.dialog = true
        this.claseF = Object.assign({}, item)
        if (item.tipo_fondo === 1) {
          this.claseF.tipo_fondo = 'Fondo'
        } else {
          this.claseF.tipo_fondo = 'Entidad financiera'
        }
      },
      close () {
        this.dialog = false
      ...
      async save () {
        if (await this.validador('formPago')) {
          this.dialog = false
          this.localLoading = true
          var send = !this.claseF.clase_fondo ? this.axios.post('clasefondo/crear', this.claseF) : this.axios.put('clasefondo/actualizar/' + this.claseF.clase_fondo, this.claseF)
          send.then(response => {
            this.localLoading = false
            if (this.claseF.clase_fondo) {
              this.$store.commit(SNACK_SHOW, {msg: 'La clase de fondo se actualizó correctamente', color: 'success'})
