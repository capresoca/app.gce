<template>
  <div>
    <v-card>
      <toolbar-list :info="infoComponent" title="Ordenes de pago" subtitle="Registro y gestión" btnplus btnplustitle="Crear Item"/>
...
<script>
  ...
  export default {
    ...
    watch: {
      'itemOrdenesDePago' (value) {
        if (value.estado === 'crear') {
          this.ordenes.splice(0, 0, value.item)
        } else if (value.estado === 'editar') {
          this.ordenes.splice(this.ordenes.findIndex(x => x.id === value.item.id), 1, value.item)
        }
      },
      'pagination.per_page' () {
    ...
    computed: {
      infoComponent () {
        return this.$store.getters.getInfoComponent('ordenesDePago')
      },
      ...mapState({
        itemOrdenesDePago: state => state.tables.itemOrdenesDePago
      }),
      pages () {
    ...
    methods: {
      reloadPage () {
        ...
        this.axios.get('pr_orden_de_pagos?per_page=' + this.pagination.per_page + '&page=' + this.pagination.current_page + '&search=%25' + this.search + '%25')
          .then((response) => {
