<template>
  <div>
    <v-dialog v-model="dialog" width="500px">
      <v-form data-vv-scope="formCajas" @submit.prevent="agregarDetalle(detalle)">
        <v-card>
          ...
          <v-container fluid grid-list-md>
            <v-layout row wrap>
              ...
              <v-flex xs12>
                <v-autocomplete
                  label="Tipo gasto"
                  v-model="detalle.tipo_gasto"
                  @change="val => detalle.pr_tipo_gasto_id = val ? val.id : null"
                  :items="complementos.tipoGastos"
                  item-value="id"
                  item-text="nombre"
                  name="tipo gastos"
                  ...
                  return-object
                  v-validate="'required'"
                  :error-messages="errors.collect('tipo gastos')">
              ...
              <v-flex xs12>
                <v-text-field
                  ...
                  :disabled="disabledValorInicial"
                  name="valor" v-validate="'required|numeric'" required
                  :error-messages="errors.collect('valor')"></v-text-field>
              </v-flex>

              <v-flex xs12>
                <v-subheader class="pa-0 ma-0">Rubro sin presupuesto</v-subheader>
                <v-switch class="ma-0 pa-0"  color="accent"
                          v-model="detalle.rubro_sin_presupuesto"  :true-value="1" :false-value="0"></v-switch>
              </v-flex>
    ...
    <v-card ref="formPresupuesto">
      <v-toolbar class="grey lighten-3 elevation-0 toolbar--dense">
        <v-toolbar-title> Presupuesto inicial de gastos </v-toolbar-title>
        <v-spacer></v-spacer>
...
<script>
  ...
  import {PRESUPUESTO_GASTOS_RELOAD_ITEM} from '../../../../store/modules/general/tables'
  import { Validator } from 'vee-validate'

  export default {
    name: 'PresupuestoInicialDeGastos',
    props: ['parametros'],
    components: {
      Loading
    },
    ...
    watch: {
      ...
      'detalle.rubro_sin_presupuesto' (value) {
        if (value === 1) {
          this.detalle.valor_inicial = 0
          this.disabledValorInicial = true
        }
        if (value === 0) {
          this.detalle.valor_inicial = null
          this.disabledValorInicial = false
        }
      }
    },
    methods: {
      imprimir (id) {
        this.axios.get('firmar-ruta?nombre_ruta=reporte-pr-gastos&id=' + id)
          .then((res) => {
            let url = res.data
      ...
      getRegisto (id) {
        ...
        this.axios.get('pr_strgastos/' + id)
          .then((response) => {
            if (response.data !== '') {
      ...
      obtenerPresupuesto (periodo) {
        ...
        this.axios.get('entidades_resolucion_gastos/' + periodo)
          .then((response) => {
            if (response.data !== '') {
          ...
          .catch(e => {
            loader.hide()
            this.$store.commit(SNACK_SHOW, {msg: 'Hay un error al traer el presupuesto. ' + e, color: 'danger'})
          })
      },
      obtenerPresupuestos () {
        ...
        this.axios.get('periodos_strgastos')
          .then((res) => {
            if (res.data !== '') {
          ...
          .catch((e) => {
            loader.hide()
            this.$store.commit(SNACK_ERROR_LIST, {expression: ' al obtener los presupuestos. ', error: e})
          })
      },
      obtenerRubros () {
        this.axios.get('rubros?tipo_rubro=Gastos')
          .then((response) => {
      ...
      submit () {
        ...
        let send = this.seEstaCreando ? this.axios.post('pr_strgastos', this.presupuesto) : this.axios.put('pr_strgastos/' + this.presupuesto.id, this.presupuesto)
        send.then(response => {
          if (this.seEstaCreando) {
            this.$store.commit(SNACK_SHOW, {msg: 'Item creado satisfactoriamente', color: 'success'})
            this.$store.commit(PRESUPUESTO_GASTOS_RELOAD_ITEM, {item: response.data.data, estado: typeRequest, key: this.parametros.key})
