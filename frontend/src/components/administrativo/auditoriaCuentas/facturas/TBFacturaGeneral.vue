<template>
  <div>
    ...
    <v-card>
      <toolbar-list :info="infoComponent" title="Soportes Facturas General" subtitle="Registro y gestión" btnplus btnplustitle="Crear registro" btnplustruncate @click="dialog = true"/>
      <v-container fluid>
        <v-layout row wrap>
...
<script>
  ...
  export default {
    ...
    computed: {
      infoComponent () {
        return this.$store.getters.getInfoComponent('facturaGeneral')
      },
      modalTitulo () {
        return !this.soporte.consecutivo_soporte ? 'Nueva factura general' : 'Editar factura: ' + this.soporte.consecutivo_soporte
      },
      pages () {
    ...
    methods: {
      reloadPage () {
        ...
        this.axios.get('facturageneral?per_page=' + this.pagination.per_page + '&page=' + this.pagination.current_page + '&search=%25' + this.search + '%25')
          .then((response) => {
      ...
      confirmaAnulacion (motivo) {
        this.localLoading = true
        this.axios.delete('facturageneral/eliminar/' + this.dialogA.registroID)
          .then(res => {
      ...
      async save () {
        if (await this.validador('formPago')) {
          this.dialog = false
          this.localLoading = true
          var send = !this.soporte.consecutivo_soporte ? this.axios.post('facturageneral/crear', this.soporte) : this.axios.put('facturageneral/actualizar/' + this.soporte.consecutivo_soporte, this.soporte)
          send.then(response => {
            this.localLoading = false
            if (this.soporte.consecutivo_soporte) {
              this.$store.commit(SNACK_SHOW, {msg: 'La factura se actualizó correctamente', color: 'success'})
              this.reloadPage() // recargo la tabla
            } else {
