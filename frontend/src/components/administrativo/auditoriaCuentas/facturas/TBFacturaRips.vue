<template>
  <div>
        ...
        <v-card>
          ...
          <v-container fluid grid-list-md>
            <v-layout row wrap>
              ...
              <v-flex xs12>
                <v-text-field v-model="soporte.nombre"
                              label="Nombre" key="nombre"
                              name="nombre" v-validate="'required|max:500|'" required
                ...
                <v-select
                  label="Obligatorio"
                  v-model="soporte.sw_obligatorio"
                  :items="valores"
                  item-text="nombre"
                  item-value="nombre"
    ...
    <v-card>
      <toolbar-list :info="infoComponent" title="Soportes Facturas RIPS" subtitle="Registro y gestión" btnplus btnplustitle="Crear registro" btnplustruncate @click="dialog = true"/>
      <v-container fluid>
        <v-layout row wrap>
        ...
        <v-data-table
          ...
          :total-items="soportes.length"
          :search="search"
          hide-actions
          class="elevation-1">
          <template slot="items" slot-scope="props">
            <td>{{ props.item.consecutivo_soporte}}</td>
              ...
              >
                ...
                <v-tooltip top>
                  <v-btn
                    ...
                    color="white"
                    slot="activator"
                    @click="eliminar(props.item.consecutivo_soporte)"
                  >
...
<script>
  ...
  export default {
    ...
    computed: {
      infoComponent () {
        return this.$store.getters.getInfoComponent('facturaRIPS')
      },
      modalTitulo () {
        return !this.soporte.consecutivo_soporte ? 'Nueva factura RIPS' : 'Editar factura: ' + this.soporte.consecutivo_soporte
      },
      pages () {
    ...
    methods: {
      reloadPage () {
        ...
        this.axios.get('facturarips?per_page=' + this.pagination.per_page + '&page=' + this.pagination.current_page + '&search=%25' + this.search + '%25')
          .then((response) => {
            ...
            this.soportes = response.data.data
      ...
      eliminar (item) {
        this.dialogA.content = '¿Está seguro de eliminar?' // botón aceptar
      ...
      confirmaAnulacion (motivo) {
        this.localLoading = true
        this.axios.delete('facturarips/eliminar/' + this.dialogA.registroID)
          .then(res => {
            this.$store.commit(SNACK_SHOW, {msg: 'La factura se eliminó correctamente', color: 'success'})
            this.reloadPage()
      ...
      async save () {
        if (await this.validador('formPago')) {
          this.dialog = false
          this.localLoading = true
          var send = !this.soporte.consecutivo_soporte ? this.axios.post('facturarips/crear', this.soporte) : this.axios.put('facturarips/actualizar/' + this.soporte.consecutivo_soporte, this.soporte)
          send.then(response => {
            this.localLoading = false
            if (this.soporte.consecutivo_soporte) {
              this.$store.commit(SNACK_SHOW, {msg: 'La factura se actualizó correctamente', color: 'success'})
              this.reloadPage()
            } else {
