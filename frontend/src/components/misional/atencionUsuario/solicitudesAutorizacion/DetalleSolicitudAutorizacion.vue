<template>
  <v-card>
    ...
    <v-toolbar dense class="elevation-0">
      ...
      <v-toolbar-title>Detalle solicitud de autorización</v-toolbar-title>
      <v-spacer/>
      <v-chip label color="white" text-color="red" dark absolute right top v-if="item">
        <span class="subheading">Consecutivo N°</span>&nbsp;
        <span class="subheading" v-text="item.consecutivo == null ? '0000' : item.consecutivo"></span>
      </v-chip>
      <v-tooltip
        top
      >
    ...
    <v-container v-if="item" fluid grid-list-sm>
      ...
      <v-layout row wrap>
        ...
        />
        <v-flex xs12 v-if="item.estado === 'Negada'">
          <v-textarea
...
<script>
  ...
  export default {
    name: 'DetalleSolicitudAutorizacion',
    props: ['parametros'],
    components: {
