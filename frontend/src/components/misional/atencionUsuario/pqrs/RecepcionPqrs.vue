<template>
  <div>
    <v-card>
      <toolbar-list :info="infoComponent" title="Recepción PQRS" subtitle="Registro y gestión" btnplus btnplustitle="Crear PQRS"/>
        ...
        <v-data-table
          ...
          <template slot="items" slot-scope="props">
            ...
            <td>
              ...
              <v-chip class="index-cero" v-if="props.item.estado != 'Respondido' && props.item.dias_para_responder < 10000" :color="colorChip(props.item.dias_para_responder)" label text-color="white" >{{ props.item.dias_para_responder }} días</v-chip></td>
            <td>
              <v-speed-dial
              ...
              >
                ...
                <v-tooltip top>
                  <v-btn
                    v-if="props.item.estado === 'Respondido'"
                    fab
                    dark
                    small
                    color="white"
...
<script>
  ...
  export default {
    ...
    watch: {
      'itemPqrs' (value) {
        if (value.estado === 'crear') {
          this.pqrsds.splice(0, 0, value.item)
        } else if (value.estado === 'editar') {
          this.pqrsds.splice(this.pqrsds.findIndex(x => x.id === value.item.id), 1, value.item)
        }
      },
    ...
    computed: {
      infoComponent () {
        return this.$store.getters.getInfoComponent('recepcionPqrs')
      },
      ...mapState({
        itemPqrs: state => state.tables.itemPqrs
      }),
      pages () {
    ...
    methods: {
      reloadPage () {
        ...
        this.axios.get('pqrsds?per_page=' + this.pagination.per_page + '&page=' + this.pagination.current_page + '&search=%25' + this.search + '%25')
          .then((response) => {
            ...
            this.pqrsds = response.data.data
      ...
      // descargarPdf (id) {
      //   this.loadingPdf = true
      //   this.axios.get(`firmar-ruta?nombre_ruta=respuesta-pqrs-pdf&id=${id}`)
      //     .then(res => {
      //       window.open(res.data)
      //       this.loadingPdf = false
      //     })
