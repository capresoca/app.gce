<template>
  <div>
        ...
        <v-card>
          ...
          <v-container fluid grid-list-md>
            <v-layout row wrap>
              ...
              <v-flex xs12>
                <v-text-field v-model="rechazo.descripcion"
                              label="Descripción" key="descripcion"
                              name="descripcion" v-validate="'required|max:500|'" required
                              :error-messages="errors.collect('descripcion')"></v-text-field>
                
                <v-text-field v-model="rechazo.codigo_interno"
                              label="Código Interno" key="código interno"
                              name="código interno" v-validate="'required|max:500|'" required
                              :error-messages="errors.collect('código interno')"></v-text-field> 
    ...
    <v-card>
      <toolbar-list :info="infoComponent" title="Rechazo Prestación Social" subtitle="Registro y gestión" btnplus btnplustitle="Crear registro" btnplustruncate @click="dialog = true"/>
      <v-container fluid>
        <v-layout row wrap>
        ...
        <v-data-table
          ...
          :total-items="rechazos.length"
          :search="search"
          hide-actions
          class="elevation-1">
          <template slot="items" slot-scope="props">
            <td>{{ props.item.consecutivo_rechazo}}</td>
            <td>{{ props.item.descripcion}}</td>
            <!-- <td>{{ props.item.tipo_preferencia | tipos(tiposPreferencias) }}</td> -->
            <td>{{ props.item.codigo_interno}}</td>
            <td>{{ props.item.sw_activo | tipos(dicotomia) }}</td>
            <td>
              <v-speed-dial
              ...
              >
                ...
                <v-tooltip top>
                  <v-btn
                    ...
                    color="white"
                    slot="activator"
                    @click="eliminar(props.item.consecutivo_rechazo)"
                  >
...
<script>
  ...
  export default {
    ...
    computed: {
      infoComponent () {
        return this.$store.getters.getInfoComponent('rechazo')
      },
      modalTitulo () {
        return !this.rechazo.consecutivo_rechazo ? 'Nuevo rechazo' : 'Editar rechazo: ' + this.rechazo.consecutivo_rechazo
      },
      pages () {
    ...
    methods: {
      reloadPage () {
        ...
        this.axios.get('rechazo?per_page=' + this.pagination.per_page + '&page=' + this.pagination.current_page + '&search=%25' + this.search + '%25')
          .then((response) => {
            ...
            this.rechazos = response.data.data
      ...
      confirmaAnulacion (motivo) {
        this.localLoading = true
        this.axios.delete('rechazo/eliminar/' + this.dialogA.registroID)
          .then(res => {
            this.$store.commit(SNACK_SHOW, {msg: 'El rechazo se eliminó correctamente', color: 'success'})
            this.reloadPage()
      ...
      editar (item) {
        ...
        this.dialog = true
        this.rechazo = Object.assign({}, item)
        this.rechazo.sw_activo = item.sw_activo.toString()
      },
      close () {
        this.dialog = false
        this.formReset()
      ...
      async save () {
        if (await this.validador('formPago')) {
          this.dialog = false
          this.localLoading = true
          var send = !this.rechazo.consecutivo_rechazo ? this.axios.post('rechazo/crear', this.rechazo) : this.axios.put('rechazo/actualizar/' + this.rechazo.consecutivo_rechazo, this.rechazo)
          send.then(response => {
            this.localLoading = false
            if (this.rechazo.consecutivo_rechazo) {
              this.$store.commit(SNACK_SHOW, {msg: 'El rechazo se actualizó correctamente', color: 'success'})
