        ...
        <template slot="items" slot-scope="props">
          ...
          <td v-if="!esPortabilidad">
            <v-autocomplete
              ...
              @change="asociarIdsEntreServicioConIps(props.index)"
...
<script>
  export default {
    ...
    methods: {
      submit () {
        this.loading = true
        this.axios.post(`afiliados/${this.afiliado.id}/servicios/sync`, {servicios_habilitados: JSON.parse(JSON.stringify(window.lodash.difference(this.idsServiciosYEntidades.filter(Boolean), this.afiliado.servicios_habilitados.map(x => x.id))))}).then(response => {
      ...
      getData () {
        ...
        this.axios.get(`afiliados/${this.afiliadoId}`)
          .then((response) => {
            this.portabilidad = response.data.data.portabilidad_activa
      ...
      asociarIdsEntreServicioConIps (index) {
        this.idsServiciosYEntidades.push({idServicio: this.serviciosHabilitar[index].id, idEntidad: this.seleccionados[index]})
        console.log(this.idsServiciosYEntidades)
      }
    }
  }
</script>
