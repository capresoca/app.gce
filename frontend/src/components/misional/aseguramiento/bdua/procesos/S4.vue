<template>
  <v-data-table
    :headers="headers"
    :items="procesosSolicitudTrasladoSubsidiado"
...
<script>
  ...
  export default {
    ...
    data () {
      return {
        procesosSolicitudTrasladoSubsidiado: [],
        pagination: {},
        tableLoading: false,
        search: '',
        headers: [
    ...
    mounted () {
      ...
      this.axios.get('tramites/pendiente?S4=1').then(res => {
        console.log('resto', res)
        this.procesosSolicitudTrasladoSubsidiado = res.data.S4
        this.pagination.totalItems = this.procesosSolicitudTrasladoSubsidiado.length
