  ...
  <div>
    <v-dialog v-model="dialog" persistent max-width="300">
        <v-card>
          ...
          <v-card-text class="subheading font-weight-light">¿Realmente desea eliminar la tutela?</v-card-text>
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn flat @click.native="dialog = false">No</v-btn>
            <v-btn color="primary" flat @click="eliminarTutela(comDiario)">Si</v-btn>
          </v-card-actions>
        </v-card>
    </v-dialog>
	  <v-card>
	    <toolbar-list :info="infoComponent" title="Tutelas MIPRES" subtitle="">
	      <template slot="actions">
	        ...
	        >
	          <v-icon class="mr-1">cloud_download</v-icon> descargar tutelas
	        </v-btn>
	      </template>
	    </toolbar-list>
	    <data-table
	      ...
	      @detalle="item => $store.commit('NAV_ADD_ITEM', { ruta: 'detalleTutelaMipres', titulo: 'Detalle Tutela MIPRES', parametros: {entidad: item, tabOrigin: $store.state.nav.currentItem.split('tab-')[1]}})"
  ...
  export default {
    ...
    data () {
      return {
        ...
        comDiario: null,
        dataTable: {
          nameItemState: 'itemMipresTutelas',
          route: 'mptutelas',
          optionsPerPage: [
            {
              text: 50,
    ...
    computed: {
      complementos () {
        return store.getters.complementosTablaTutelas
      },
      infoComponent () {
    ...
    methods: {
      ...
      eliminarTutela (comdiario) {
        this.dialog = false
        // delete comdiario.show
        console.log(comdiario.id)
        console.log('tutelamipres/eliminar/' + comdiario.id)
        this.axios.delete('tutelamipres/eliminar/' + comdiario.id)
          .then((response) => {
