...
<!--          <v-flex xs4 style="height: 66px !important;">-->
<!--        </v-flex>-->
        <slot name="actions"></slot>
        <v-flex xs1 style="height: 66px !important;" v-if="conRegistro">
        ...
        <v-flex xs11 sm6 md5 lg3 style="height: 66px !important;" v-if="conColumnas">
          <v-select
        ...
        <v-flex xs12 sm6 md3 lg2 style="height: 66px !important;" v-if="conRegistro">
          <v-select
        ...
        <v-flex xs12 sm12 md6 lg3 style="height: 66px !important;" v-if="conBuscador">
          <v-text-field
      ...
      <v-expansion-panel v-if="value.advanceFilters && filterStatic" v-model="panel" readonly expand>
  ...
  export default {
    ...
    methods: {
      getParametros (data) {
        ...
        this.parametrosocial = this.clone(data)
      },
      deselectAll () {
        this.selecteds = []
      },
      ...
      async reloadPage () {
        if (this.activePetition) {
          ...
          this.axios.get(this.value.route + (this.value.route.indexOf('?') > -1 ? '&' : '?') + 'per_page=' + this.value.pagination.per_page + stringSort + '&page=' + this.value.pagination.current_page + '&search=%25' + ((this.value.search === null || typeof this.value.search === 'undefined') ? '' : this.value.search) + '%25',
            this.parametrosocial !== null ? {params: this.parametrosocial} : null)
            .then((response) => {
              this.filtrado = true
