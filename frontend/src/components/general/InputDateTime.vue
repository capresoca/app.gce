    ...
    <v-card>
      ...
      <v-card-actions>
        ...
        >
          ...
          <v-btn color="primary darken-1" flat @click="okHandler">{{okText}}</v-btn>
        </slot>
      </v-card-actions>
    </v-card>
  </v-dialog>
  ...
  export default {
    ...
    methods: {
      okHandler () {
        ...
        this.$emit('input', this.moment(this.selectedDatetime).format(this.format))
      },
      clearHandler () {
        this.display = false
        this.activeTab = 0
