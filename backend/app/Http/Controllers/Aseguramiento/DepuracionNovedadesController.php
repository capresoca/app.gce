...
class DepuracionNovedadesController extends Controller
{
    ...
    public function store(DepuracionRequest $request)
    {
        try {
            ...
            /*$novedadNueva       = new AsNovtramite();
            
            foreach ($novedadNueva->getFillable() as $cam) {
                $novedadNueva->{$cam}           = $novedad->{$cam};
            }
    ...
    public function validarSiNegAbierto (AsNovtramite $novtramite) {
        try {
            if (! is_null($novtramite->consecutivo_vigencia)) {
                ...
                //Log::info('Info periodo vigente: ',array($liquidacion));
