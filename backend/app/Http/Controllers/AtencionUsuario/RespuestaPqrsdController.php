...
class RespuestaPqrsdController extends Controller
{
    ...
    {
        $respuesta = $pqrsd->respuesta()->create($request->except('files','soluciono_queja'));

        $this->subirAnexos($respuesta,$request->file('files'));

        $pqrsd->soluciono_queja = $request->soluciono_queja;
    ...
    {
        $respuesta->update($request->except('files','soluciono_queja'));
