...
class CeAseguradora extends Model implements Auditable {
	use PimpableTrait;
	use \OwenIt\Auditing\Auditable;

	protected $fillable = ['codigo', 'nombre', 'gn_tercero_id', 'direccion', 'telefono'];
		...
		if ($constraint->getOperator() === Constraint::OPERATOR_LIKE || $constraint->getOperator() === Constraint::OPERATOR_EQUAL) {
			$builder->where(function ($query) use ($constraint) {
				$query->where('codigo', $constraint->getOperator(), $constraint->getValue())
					...
					->orWhere('telefono', $constraint->getOperator(), $constraint->getValue());
