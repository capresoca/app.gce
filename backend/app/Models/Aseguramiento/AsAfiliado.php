...
class AsAfiliado extends Model implements Auditable
{
    ...
    {
        return $this->belongsTo(AsFormafiliacione::class,'as_afiliado_id');
    }
    ...
    public function zona_dnp()
    {
        return $this->belongsTo(GnZona::class, 'zona_dnp');
    }
...
//        $fecha_array = substr($this->attributes['fecha_nacimiento'], 0, 4) . substr($this->attributes['fecha_nacimiento'], 5, 2) . substr($this->attributes['fecha_nacimiento'], 8, 2);
...
//    }
    ...
    {
        //Log::info('Fecha afiliacion revisar: ',array($this->fecha_afiliacion));
        if (is_null($this->fecha_afiliacion)) return null;
        
        $fecha = new Carbon();
        $fecha->parse($this->fecha_afiliacion);
    ...
    protected function processSearchFilter(Builder $builder, Constraint $constraint)
    {
        ...
        if ($constraint->getOperator() === Constraint::OPERATOR_LIKE || $constraint->getOperator() === Constraint::OPERATOR_EQUAL) {
            ...
            })->where(function ($query) use ($builder, $constraint) {
    ...
    public function getProcessAffiliatesAttribute($key)
    {
        try {
            ...
            $estado         = EstadoAfiliadoTrait::consultantEstate($afiliadoId,$this->estado_traslado);
...
//    public function setFechaNacimientoAttribute($key, $value)
//    {
//       $this->attributes['fecha_nacimiento'] = Carbon::parse($value)->format('Y-m-d');
//    }
//
//    public function setFechaExpedicionAttribute($key, $value)
//    {
//       $this->attributes['fecha_expedicion'] = Carbon::parse($value)->format('Y-m-d');
//    }
//
//    public function setFechaAfiliacionAttribute($key, $value)
//    {
//       $this->attributes['fecha_afiliacion'] = Carbon::parse($value)->format('Y-m-d');
//    }
}
