...
class CmFactura extends Model implements Auditable
{
    ...
    public function getExistenGlosasAttribute()
    {
        $glosasFacturas = $this->glosas()->get()->count();
        $servicios = $this->servicios()->with('glosas')->get()->toArray();
        $glosaServicio = [];

        foreach ($servicios as $key => $servicio) {
            if (count($servicio['glosas']) > 0) {
                array_push($glosaServicio, $servicio);
            }
        }
        $glosasServicios = count($glosaServicio);

        if (($glosasFacturas <= 0) || ($glosasServicios <= 0)) {
            return false;
        }
