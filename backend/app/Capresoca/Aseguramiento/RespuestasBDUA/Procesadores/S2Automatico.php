...
class S2Automatico extends Procesador implements ProcesadorCSVInterface
{
    protected $empresa_subsidiado=null;
    ...
    public function procesar()
    {
        ...
        if(!$this->respuesta){
            $this->pushMonitor('No se puede crear respuesta archivo S2 no encontrado',$this->proceso,'error--text');
            return;
        }
        $this->empresa_subsidiado=GnEmpresa::first()->codhabilitacion_subsid;
        $this->procesadas = 0;
        $this->aplicadas = 0;
            ...
            try{
                ...
                $tramite = AsSoltraslado::where('as_afiliado_id',$afiliado->id)
                    ...
                    if($tramite){
                        ...
                        $tramite->save();
                        $this->pushMonitor('Se cambio el estado del tramite traslado Subsidiado a VALIDADO y se aplicaron los cambios correspondientes: '. $regRespuesta->respuesta, $this->proceso,'console-success');
                    }else{
    ...
    private function actualizardatosR2(AsAfiliado $afiliado, $row){
        ...
        if ($this->empresa_subsidiado == $row[1]) {
