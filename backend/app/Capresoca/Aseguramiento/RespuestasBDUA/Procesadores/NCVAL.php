...
class NCVAL extends Procesador implements ProcesadorCSVInterface
{
    ...
    public function procesar()
    {
        ...
        foreach ($this->data as $novedad) {
            try{
                $this->procesadas ++;
                $log = 'Procesando novedad Contributiva: '.$novedad[0];
                ...
                $this->pushMonitor('Se proceso la novedad Contributiva: '.$novedad[0],$this->proceso,'console-success');

            }catch (\Exception $e)
            {
                Log::error($e->getFile().'  Linea:  '.$e->getLine());
