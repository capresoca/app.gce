...
class MSVAL extends Procesador implements ProcesadorCSVInterface
{
    use MSTrait;
    ...
    public function procesar()
    {
        $this->procesadas = 0;
        $this->aplicadas = 0;
        $this->respuesta = $this->crearRespuestaMSVAL('Valido');
        $this->proceso = $this->respuesta->archivo->proceso;

        if(!$this->respuesta) {
            $log = 'No se encuentra MS VAL';
        ...
        $registros      = DB::select("select d.informacion,d.tipo_identificacion_afiliado,d.numero_identificacion_afiliado as identificacion,
                                            ...
                                            a.fecha_nacimiento,if(a.genero=1,'F','M') as genero, a.consecutivo_afiliado, f.id
                                                 from log_ms_encabezado e
                                                    ...
                                                    enc.consecutivo_log_ms = d.consecutivo_log_ms AND enc.tipo = ".ETipoBdua::getIndice(ETipoBdua::MS_VAL)->getId()."
                                                    inner join af_afiliado_ms a on
                                                    a.consecutivo_ns = d.consecutivo_ms
                                                    inner join as_formafiliaciones f on
                                                    f.as_afiliado_id = a.consecutivo_afiliado and a.tipo_traslado = 'MS'
                                                    where a.consecutivo_vigencia = {$this->vigencia}  and a.tipo_traslado = 'MS' ");
        Log::info('Sql analizar: '."select d.informacion,d.tipo_identificacion_afiliado,d.numero_identificacion_afiliado as identificacion,
                                            a.primer_apellido,a.segundo_apellido,a.primer_nombre,a.segundo_nombre,
                                            a.fecha_nacimiento,if(a.genero=1,'F','M') as genero, a.consecutivo_afiliado, f.id
                                                 from log_ms_encabezado e
                                                    inner join log_ms_detalle d on
        ...
        foreach ($registros as $r)
        {
            try {
                ...
                $afiliado->save();
                $tramite = $this->getTramite($afiliado, $afiliacion[12]);
                ...
                if ($accion) {
                    $this->aplicadas++;
                    $this->pushMonitor('Se activo el afiliado: ' . $afiliado->tipo_identificacion . ' ' . $afiliado->identificacion, $this->proceso, 'console-success');
                }
    ...
    private function crearRespuestaMSVAL($tipo) {
