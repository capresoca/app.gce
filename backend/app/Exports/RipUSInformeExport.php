...
class RipUSInformeExport implements FromView, WithEvents, WithCharts
{
    ...
    {
        return view('maatwebsitExcel.reporteRIPUS', ['rips' => $this->data]);
    }
    ...
    public function firstSheet (Worksheet $spreadsheet)
    {
        try {
            ...
            foreach ($this->data as $key => $tutela){
                $spreadsheet->getStyle('A'. ($key + 2) .':V'.($key+2))
