...
class CarbonColombia extends Carbon
{
    ...
    public function isBussinessDay()
    {
        return !$this->isHoliday() && $this->isWeekday();
    }

    public function bussinessDayOfMonth()
    {
        $firstDay = $this->firstBussinessDayOfMonth();

        if(!$this->isBussinessDay()) return 0;

        if($firstDay === $this) return 1;

        if($firstDay->greaterThan($this)) {
            return $firstDay->diffInDays($this);
        }

        return $this->diffInBussinessDays($this->firstBussinessDayOfMonth()) + 1;
    }

    public function firstBussinessDayOfMonth()
    {
        $primero = CarbonColombia::parse($this->toDateString())->firstOfMonth();

        while (!$primero->isBussinessDay()){
            $primero->addDay();
        }
