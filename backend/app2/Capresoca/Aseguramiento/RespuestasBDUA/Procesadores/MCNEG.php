...
class MCNEG extends ProcesadorNegados implements ProcesadorCSVInterface
{
    use MSTrait;
    ...
    public function procesar() {
        ...
        foreach ($this->data as $fila) {
            try {
                $this->procesadas++;
                $afiliado = $this->getAfiliado($fila[3], $fila[4]);
                $tramite = $this->getTramite($afiliado, $fila[18]);
