...
class FacturaController extends Controller
{
    ...
    public function asignacionFactura(Request $request)
    {
        ...
        foreach ($request->facturas as $key => $factura) {
            ...
            if ($fact->auditores()->count() >= 1) {
                foreach ($request->auditores as $auditore) {
                    foreach ($fact->auditores as $key => $audit) {
                        if ($auditore !== $audit->id) {
                            $this->siReasigna($audit, $fact, 1);
                            $this->attachAuditorFactura($fact, $auditore, 1);
                        }

                        if ($auditore === $audit->id) {
                            $this->siReasigna($audit, $fact, 2);
                        }
                    }
                }
            } else {
                $rad = CmRadicado::whereId($fact->cm_radicado_id)->first();
                if ($rad['estado'] === 'Radicado') {
                    $rad->estado = 'En Auditoria';
                    $rad->save();
                }
                foreach ($request->auditores as $auditore) {
                    $this->attachAuditorFactura($fact, $auditore, 2);
                }
            }
        ...
        $radicados = CmRadicado::with('facturas')->get();
        ...
        return response()->json(['data' => new FacturasResource($data), 'status' => 'ok'])->setStatusCode(200);
    }
    ...
    {
        $fact->auditores()->attach($auditore, [
            'gs_usuario_id' => Auth::user()->id,
            'estado' => '1',
     ...
     */
    private function getAllRegistros(): \Illuminate\Http\Resources\Json\AnonymousResourceCollection
    {
