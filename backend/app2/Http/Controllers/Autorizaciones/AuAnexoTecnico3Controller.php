...
class AuAnexoTecnico3Controller extends Controller
{
    public function index()
    {
        if (Input::get('per_page')) {
            return AnexoTecnico3Resource::collection(
                AuAnexot3::funcionario()
        ...
        foreach ($request->detalles as $key => $detail) {
            ...
            if ($request->ind === '3') {
                if ($detail['ind'] === 1 && ! isset($detail['negacion'])) {
                    ...
                    $id_afiliado = $anexot3->afiliado['id'];
                    $codigo = $detail['codigo'];
                    $cuota_moderadora = DB::selectOne("SELECT b.codigo, 
                                                         if(e.as_tipafiliado_id = 1,e.ibc,e1.ibc) AS ibc, 
                                                         f.copa,
                                                         e.as_tipafiliado_id,
                                                         if(e.as_tipafiliado_id = 1,if(e.ibc <= (877803*2) or e.ibc is null,'A',if(e.ibc <= (877803*5),'B','C')),if(e1.ibc <= (877803*2) OR e1.ibc is null,'A',if(e1.ibc <= (877803*5),'B','C')))  as categoria, 
                                                         if(e.as_tipafiliado_id = 1,if(e.ibc <= (877803*2) or e.ibc is NULL,'3400',if(e.ibc <= (877803*5),'13500','35600'))
                                                           ,if(e1.ibc <= (877803*2) OR e1.ibc is NULL,'3400',if(e1.ibc <= (877803*5),'13500','35600')))  as valorCopago
                                                FROM au_anexot3s AS a
                                                LEFT JOIN au_anexot31s  AS b ON b.au_anexot3s_id= a.id
                                                LEFT JOIN au_anexot3regs AS d ON d.au_anexot3_id = a.id
                                                LEFT JOIN as_afiliados AS e ON e.id = a.as_afiliado_id
                                                LEFT JOIN as_afiliados AS e1 ON e1.id = e.cabfamilia_id
                                                LEFT JOIN refcups AS c ON c.codigo = b.codigo
                                                LEFT JOIN refcobertura AS f ON f.codigo = c.cober
                                                LEFT JOIN refntcie10gestantes AS g ON g.codigo = a.dPrin
                                                WHERE f.copa='1' AND e.id = '{$id_afiliado}' AND c.codigo = '{$codigo}'
                                                AND e.as_regimene_id = '1' AND g.codigo IS NULL AND a.ubic != '3' 
                                                AND ( year(curdate())-YEAR(e.fecha_nacimiento) ) - (right(curdate(),5)<RIGHT(e.fecha_nacimiento,5)) > 1");


                    if ((! $this->validateCubrioCopagoAnio($anexot3->afiliado, 1))
                        || (! $this->noCopaOCuota($anexot3))
                        || (! $this->isNivelOorN($anexot3->afiliado) && $anexot3->afiliado['as_regimene_id'] === 2)) {
...
//            $detail['copago'] = ($valorUsuario === 0) ? ($valorUsuario = 0) : ((double)$valorUsuario);
            ...
            if ($detail['id'] !== null) {
                foreach ($detalles as $detalle) {
                    if ($detalle->id === $detail['id']) {
                        $ref = AuAnexot31::with('usuarioValida', 'negacion')
                            ->where('id', $detalle->id)->first();
    ...
    public function getValorTotalIps($anexo, $value)
    {
        ...
        $suma = $anexot->where('pAut', '=', $value->id)
            ->where('ind', '=', 1)
            ->whereNotIn('cAut', [null, 0,])
            ->sum(function ($query) {
                return ($query->valor * $query->cAut);
            });

        $copago = $anexot->where('pAut', '=', $value->id)
            ->where('ind', '=', 1)
            ->whereNotIn('cAut', [null, 0,])->sum(function ($query) {
            return ($query->copago);
        });
    ...
    {
        return $cobertura['copa'] === '1' && $anexotCabeza['afiliado']['nivel_sisben'] >= '2'
            && $anexotCabeza['afiliado']['edad'] >= 1
            && ($anexot31['modSer'] === '3' || $anexot31['servicio']['copago'] === '1');
    }
    ...
    {
        return ($afiliado->sexo['abreviatura'] === 'M' && $genero === 'F')
            || ($afiliado->sexo['abreviatura'] === 'F' && $genero === 'M');
    }
