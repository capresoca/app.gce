    ...
    <main>
        ...
        <section id="info_basica" style="margin-top: -16px">
            <table>
                ...
                <tr>
                    ...
                    s
                    <td colspan="2">Fecha vencimiento:
                        {{strftime('%d/%m/%Y', strtotime( \Carbon\Carbon::parse($autaprobada->fS1)->addMonths(6)))}}</td>
                ...
                <tr>
                    ...
                    ?  $autaprobada->afiliado->identificacion_completa : 'NO REGISTRA' }}</td>
                ...
                <tr>
                    ...
                    <td colspan="2">Dirección: {{$autaprobada->entidad_ips->direccion_sede
                    ? $autaprobada->entidad_ips->direccion_sede : 'NO REGISTRA' }}
                        {{ $autaprobada->entidad_ips->municipio
                        ? $autaprobada->entidad_ips->municipio->nombre : 'NO REGISTRA' }}</td>
                <tr>
        ...
        <section id="detalles" style="margin-top: 10px">
            <table style="width: 100%">
                <tr>
{{--                    <td colspan="4">Prestador Asignado: <strong> {{$servicios[$i]['entidad']--}}
{{--                ? $servicios[$i]['entidad']['nombre'] : 'NO REGISTRA' }}</strong></td>--}}
{{--                    <td colspan="3">Dirección: <strong> {{$servicios[$i]['entidad']--}}
{{--                ? $servicios[$i]['entidad']['direccion_sede'] : 'NO REGISTRA' }} {{$servicios[$i]['entidad']--}}
{{--                ? $servicios[$i]['entidad']['municipio']['nombre'] : 'NO REGISTRA' }}</strong></td>--}}

                    <td colspan="4">Prestador Asignado: <strong> {{$servicio['entidad']
                ? $servicio['entidad']['nombre'] : 'NO REGISTRA' }}</strong></td>
                    <td colspan="3">Dirección: <strong> {{$servicio['entidad']
                ? $servicio['entidad']['direccion_sede'] : 'NO REGISTRA' }} {{$servicio['entidad']
                ? $servicio['entidad']['municipio']['nombre'] : 'NO REGISTRA' }}</strong></td>
                ...
                <tr style="font-weight: bold; text-align: center">
                    ...
                    <td style="width: 10%">{{ $autaprobada->afiliado
                    ?  ($autaprobada->afiliado['as_regimene_id'] === 1 ? 'Cuota M.' : 'Copago')
                    : 'NO REGISTRA' }}</td>
                    <td style="width: 10%">Valor EPS</td>
                    <td>Observaciones</td>
                </tr>
                <tbody style="font-size: 8px">
                @foreach($autaprobada->detalles as $detalle)
                    @foreach($servicio['detallesPermitidos'] as $idPerm)
                        @if($detalle->id === ((int) $idPerm))
                            @if($detalle['pAut'] === $servicio['entidad_id']
                            && ($detalle->ind === 1 && ($detalle->cAut !== null || $detalle->cAut !== 0)))
                                <tr>
                                    ...
                                    <td style="text-align: right">
                                        <span>{{ $detalle->valor !== null
                                        ? '$ '. (number_format(($detalle->valor * $detalle->cAut), 2, ',', '.'))
                                        : "$ 0,00" }}</span>
                                    </td>
                ...
                <tr style="font-weight: bold;">
                    ...
                    <span style="float: right">
                        {{'$ ' . number_format($servicio['valor_copago'], 2, ',', '.')}}
                    </span>
                    </td>
                    <td style="border-top: double gray 3px">
                    <span style="float: right">
                        {{ '$ ' . number_format($servicio['valor_x_entidad'], 2, ',', '.')}}
                    </span>
                    </td>
