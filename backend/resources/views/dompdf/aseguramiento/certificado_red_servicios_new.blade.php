...
<main>
    ...
    <br>
    @if($afiliado->estado_afiliado->nombre == 'Activo' && isset($grupoIps))
        <table>
            <tr>
                <td colspan="5" style="font-size: 2.5mm !important;">
                     {!! "<b> Grupo: </b>" . strtoupper($grupoIps['descrip']) !!}
                </td>
            </tr>
            <tr>
                <th style="padding-top: 1mm !important;">Prestador</th>
                <th style="padding-top: 1mm !important;">Dirección</th>
                <th style="padding-top: 1mm !important;">Telefono</th>
                <th style="padding-top: 1mm !important;">Ciudad</th>
                <th style="padding-top: 1mm !important;">Primaria</th>
            </tr>
            @foreach($grupoIps['prestadores'] as $key =>$prestador)
                <tr>
                    <td>
                        <span style="display: block !important; padding-top: 0; margin-bottom: 0.1mm">{!! strtoupper($prestador['entidad']['nombre']) !!}</span>
                        <span style="display: block !important;">{!! "Código: <b>" . $prestador['entidad']['cod_habilitacion'] . "</b> " !!}</span>
                    </td>
                    <td>{!! $prestador['entidad']['direccion_sede'] !!}</td>
                    <td>{!! $prestador['entidad']['telefono_sede'] !!}</td>
                    <td>{!! $prestador['entidad']['municipio']['nombre'] !!}</td>
                    <td style="padding-left: 5mm">{!! ($prestador['primaria'] === 1) ? 'X' : '' !!}</td>
                </tr>
            @endforeach
        </table>
    @else
        @if($afiliado->estado_afiliado->nombre == 'Activo' && !isset($grupoIps))
            <h3 style="color: red">NO SE ASIGANDO EL GRUPO DE IPS ({{strtoupper($afiliado->estado_afiliado->nombre)}})</h3>
        @else
            <h3 style="color: red">SIN GRUPO DE IPS ({{strtoupper($afiliado->estado_afiliado->nombre)}})</h3>
        @endif
    @endif
    <br>
    <br>
    ...
    <div style="text-align: center; width: 100%; margin-top: 80px;">
        <p style="width: 50%; margin: 0 auto; border-top: black solid 1px; text-align: center "><strong>PROCESO MISIONAL
                DE ASEGURAMIENTO</strong></p>
