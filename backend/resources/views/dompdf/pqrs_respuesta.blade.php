...
<main>
    ...
    <br>
    <br>
    <p>ASUNTO: Respuesta Queja No. <strong>{{$pqrs->consecutivo_externo ? $pqrs->consecutivo_externo:  "NR"}}</strong>,
        Consecutivo Interno No. <strong>{{ str_pad($pqrs->consecutivo,6,"0",STR_PAD_LEFT)}}</strong></p>
    <br>
    <p>{!!'<strong>Fecha Inicio: </strong>'. strftime('%d de %B del %Y', strtotime($pqrs->fecha_recepcion)) .' | <strong> Fecha Respuesta: </strong>' . strftime('%d de %B del %Y', strtotime($pqrs->respuesta['fecha_respuesta']))!!}</p>
    <br>
    <p id="contenido">
        {!!$pqrs->respuesta->respuesta!!}
    </p>
    <br>
    <p><strong>FRENTE A CUALQUIER DESACUERDO FRENTE A LA DECISIÓN TOMADA, PODRÁN ELEVAR CONSULTA ANTE EL ENTE
            TERRITORIAL DEPARTAMENTAL Y/O MUNICIPAL Y LA SUPERINTENDENCIA NACIONAL DE SALUD</strong></p>
    <br>
    <br>
    <p>Cordialmente.</p>
    <div id="firmas">
